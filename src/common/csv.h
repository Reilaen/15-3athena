...
enum csv_parse_flags {
    ...
    CSV_FLAG_BRACE_AWARE    = 0x04, // {}-Dont seprate on {} (für Scripts)
    CSV_FLAG_COMMENT_SKIP   = 0x08, // Ignore lines starting with comments
 ...
 * @return              CSV_OK if successfull else error code
 */
int csv_parse_line(char *line, csv_result_t *result, int max_fields, char delimiter, uint32 flags);

/**
 * Read a field as int
 */
int csv_get_int(const csv_result_t *res, int idx, int default_val);

/**
 * Read field as string (NULL-safe)
 */
const char* csv_get_str(const csv_result_t *res, int idx);

/**
 * Field als Boolean (0/1, true/false, yes/no)
 */
bool csv_get_bool(const csv_result_t *res, int idx, bool default_val);
