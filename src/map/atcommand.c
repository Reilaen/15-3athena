...
ACMD_FUNC(guildstorage)
{
	...
	case GSTORAGE_OPEN:
		clif_displaymessage(fd, msg_txt(sd, 829)); // Guild storage opened.
		break;
	...
	case GSTORAGE_NO_STORAGE:
		clif_displaymessage(fd, msg_txt(sd, 827)); // The guild does not have a guild storage.
		return -1;
	case GSTORAGE_NO_PERMISSION:
		clif_displaymessage(fd, msg_txt(sd, 828)); // You do not have permission to use the guild storage.
		return -1;
	}
	return 0;
}
